...
## Acceptance criteria
- [ ] A 409 from `POST /pod/administrators` (`{ "error": { "code": ..., "message": "Email already exists" } }`, see `PodAdministratorController.kt:181,218`) is shown as the email field's error text and the dialog remains open with the entered values.
...
## Contract
`POST /pod/administrators` → `409 Conflict` with the standard error body when the email exists (`specs/contracts/api.md`, Pod administrators). No change.
